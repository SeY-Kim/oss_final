    ...
    $.ajax({
        ...
        .then(function (response) {
            ...
            if(t<6)
                ...
                closet3 =  $('<p>').html("추천 OOTD 💁 "+"<img src='img/01.png' width='200' height='100' />");}
            else if(6<=t && t<10)
                {closet1 = $('<p>').html("히트텍을 입는 건 어떤가요? 코트나 가죽자켓, 니트, 두꺼운 바지를 입는 게 좋을 것 같아요!🧥");
                closet3 =  $('<p>').html("추천 OOTD 💁 "+"<img src='img/02.png' width='200' height='100' />");}
            else if(10<=t && t<12)
                {closet1 = $('<p>').html("트렌치 코트나 자켓과 같은 그리 두껍지 않은 아우터를 입고 청바지나 면바지를 입어도 좋을 것 같아요👖");
                closet3 =  $('<p>').html("추천 OOTD 💁 "+"<img src='img/03.png' width='200' height='100' />");}
            else if(12<=t && t<17)
                {closet1 = $('<p>').html("청자켓과 같은 얇은 자켓, 가디건, 맨투맨 , 후드티, 니트를 입어도 딱 좋을 것 같아요><🧶");
                closet3 =  $('<p>').html("추천 OOTD 💁 "+"<img src='img/04.png' width='200' height='100' />");}
            else if(17<=t && t<20)
                {closet1 = $('<p>').html("얇은 니트나 얇은 자켓, 가디건, 기모가 없는 맨투맨이나 후드티만 입어도 충분해요><👟");
                closet3 =  $('<p>').html("추천 OOTD 💁 "+"<img src='img/05.png' width='200' height='100' />");}
            else if(20<=t && t<23)
                {closet1 = $('<p>').html("얇은 긴팔이나 얇은 가디건 또는 블라우스에 슬랙스 같은 면바지를 입어서 오늘은 좀 가볍게 외출해볼까요?🌱");
                closet3 =  $('<p>').html("추천 OOTD 💁 "+"<img src='img/06.png' width='200' height='100' />");}
            else if(23<=t && t<27)
                {closet1 = $('<p>').html("시원하게!! 반팔티, 얇은 셔츠에 면바지 또는 반바지로 시원하게 입어요!👕");
                closet3 =  $('<p>').html("추천 OOTD 💁 "+"<img src='img/07.png' width='200' height='100' />");}
            else if(t >=27)
                {closet1 = $('<p>').html("가만히 있어도 땀이 줄줄 날 거 같으니 무조건 시원하게 !! 갖고 있는 옷 중에 가장 시원한 옷들로 골라보세요!! 반팔, 반바지, 민소매티 린넨 소재의 옷도 좋을 거 같아요!🩳");
                closet3 =  $('<p>').html("추천 OOTD 💁 "+"<img src='img/08.png' width='200' height='100' />");}
